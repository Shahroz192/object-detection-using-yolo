    ...
    for year in years:
        for image_set in sets:
            ...
            for image_id in tqdm(image_ids, desc=f"Processing {year} {image_set}"):
                # XML annotation path
                xml_file = (
                    Path(voc_path) / f"VOC{year}" / "Annotations" / f"{image_id}.xml"
                )

                # Image file path
                image_file = (
                    Path(voc_path) / f"VOC{year}" / "JPEGImages" / f"{image_id}.jpg"
                )
                ...
                txt_output_path = (
                    output_path / "labels" / output_dir / f"{image_id}.txt"
                )
                img_output_path = (
                    output_path / "images" / output_dir / f"{image_id}.jpg"
                )
